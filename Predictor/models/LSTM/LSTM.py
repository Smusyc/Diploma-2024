...
def main():
    ...
    print(f'LSTM [start]: {sys.argv[2]}')
    
    end = int(sys.argv[3])
    print(f'LSTM [end]: {sys.argv[3]}')
    ...
    print(f'LSTM [dataset]: {sys.argv[1]}')
    

    print(f'LSTM [X_series]: ',df['X'][start:end].head())
    print(f'LSTM [Y_series]: ',df['Y'][start:end].head())
    print(f'LSTM [Z_series]: ',df['Z'][start:end].head())
    ...
    print(f'LSTM [X_series_difference]: {X_series.head()}; Index = {(X_series.index)[0]}')
    
    Y_series = diff_dataset_for_training['Y']
    print(f'LSTM [Y_series]: {Y_series.head()}; Index = {(Y_series.index)[0]}')
    
    Z_series = diff_dataset_for_training['Z']
    print(f'LSTM [Z_series]: {Z_series.head()}; Index = {(Z_series.index)[0]}')
    ...
    model = keras.models.load_model('./models/LSTM/LSTM_L2_U50_E5_v1_X')
    ...
    print(f'LSTM [X_series_pred]: {X_series_pred.size}')

    model = keras.models.load_model('./models/LSTM/LSTM_L2_U50_E5_v1_Y')
    ...
    print(f'LSTM [Y_series_pred]: {Y_series_pred.size}')
    
    model = keras.models.load_model('./models/LSTM/LSTM_L2_U50_E5_v1_Z')
    ...
    print(f'LSTM [Z_series_pred]: {Z_series_pred.size}')
    ...
    print(f'====LSTM cascade result_df_exel_2: ',result_df_exel_2.head())
    ...
    print(f'====LSTM start time: ',df['time'].iloc[start])
