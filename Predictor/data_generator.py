...
Original file is located at
    https://colab.research.google.com/drive/1Mv6nVsOISQasvxPDVmevjrLN7G89spjr
"""
...
def new_calculation_flight_old(T, start_t, delta_t, start_boost, start_speed, start_X, start_Y, start_angle, max_angle_change, max_speed, max_pos_boost, max_neg_boost, min_speed, prob_boost_change, prob_angle_change, max_height_speed):
  prob_boost = [0+prob_boost_change/2, 1-prob_boost_change/2]
  prob_angle = [0+prob_angle_change/2, 1-prob_angle_change/2]


  N = int(T/delta_t)      #Количество отметок
  old_X=start_X
  new_X = 0
  old_Y=start_Y
  new_Y = 0
  ...
  for j in range(N-1):
    trajectory_vect.append( (old_X, old_Y, start_t) )
    start_t+=delta_t
    #Angle
    rr = random.random()
    if(rr<prob_angle[0]):
      ...
      new_angle = old_angle + (max_angle_change * (rr/prob_angle[1]))

    #Boost
    rr = random.random()
    if(rr<prob_boost[0]):
      new_boost = old_boost - (rr/prob_boost[0])
      if(new_boost<max_neg_boost):
        new_boost = max_neg_boost
    elif(rr>prob_boost[1]):
      new_boost = old_boost + (rr/prob_boost[1])
...
def new_calculation_flight_old2(T, start_t, delta_t, start_boost, start_speed, start_X, start_Y, start_angle, max_angle_change, max_angle, max_speed, max_pos_boost, max_neg_boost, min_speed, prob_boost_change, prob_angle_change):
  ...
  for j in range(N-1):
    ...
    prob_boost = [0+prob_boost_change/2, 1-prob_boost_change/2]
    prob_angle = [0+(prob_angle_change/2)*(1-abs((max_angle-abs(old_angle))/max_angle)), 1-(prob_angle_change/2)*(1-abs((max_angle-abs(old_angle))/max_angle))]
    #prob_angle = [0+0.1+(prob_angle_change/2)*(1-abs((max_angle-abs(old_angle))/max_angle)), 1-0.1-(prob_angle_change/2)*(1-abs((max_angle-abs(old_angle))/max_angle))]
    #print(f"prob_angle[0]={(1-abs((max_angle-abs(old_angle))/max_angle))}; prob_angle[1]={abs((max_angle-abs(old_angle))/max_angle)}\n")
    #print(f"prob_angle[0]={prob_angle[0]}; prob_angle[1]={prob_angle[1]}\n")
    ...
    if(rr<prob_angle[0]):
      new_angle = old_angle - (max_angle_change * (rr/prob_angle[0]))
    #elif(rr>(prob_angle[1]*((max_angle-old_angle)/max_angle))):
    elif(rr>prob_angle[1]):
...
def dubbling_maker(np_trajectory, prob_dubbling):#В timeshift_wide помещаем значение delta_t
...
def timeshift_maker(np_trajectory, prob_timeshift, timeshift_wide):#В timeshift_wide помещаем значение delta_t
  ...
  for i in range(np_trajectory.shape[0]):
    rr = random.random()
    if(rr<prob[0]):
      #np_trajectory[i][3] -= timeshift_wide* (rr/prob[0])
      np_trajectory[i][3] -= timeshift_wide
    elif(rr>prob[1]):
      #np_trajectory[i][3] += timeshift_wide* ((rr-prob[1])/(1-prob[1]))
      np_trajectory[i][3] += timeshift_wide

  return np.array(np_trajectory)

def missing_maker(np_trajectory, prob_missing):#В timeshift_wide помещаем значение delta_t
...
def slope_zero(new_np_trajectory):
  ...
  for i in range(new_np_trajectory.shape[0]):
    ...
    #start_point = new_np_trajectory[i][0:2]
    #end_point = new_np_trajectory[i][0:2]
    #print(f"start_point = {start_point}")
    #print(f"end_point = {end_point}")
    #vector_point = start_point - end_point
    ...
    #print(vector_point[0])
    #print(end_point_y - start_point_y-start_point_x)
    vector_point = [end_point_x-start_point_x, end_point_y - start_point_y]
    #print(vector_point[0])
    #print(vector_point[1])
    ...
    #print(f"tang_ray = {tang_ray}; angle_rad_ray = {angle_rad_ray}; tang_point = {tang_point}; angle_rad_point = {angle_rad_point}; angle_rad_diff = {angle_rad_diff}")
    #print(f"Was = [{new_np_trajectory[i][0]}, {new_np_trajectory[i][1]}]")
    #print(f"Sin/Cos = [{math.sin(angle_rad_diff)}, {math.cos(angle_rad_diff)}]")
    ...
    #print(f"Now = [{new_np_trajectory[i][0]}, {new_np_trajectory[i][1]}]")
...
def save_generated_data(result_df):
  ...
  #files = ["output_1.xlsx", "output_2.xlsx", "output_4.xlsx", "output_5.xlsx"]
  max_num_file = 0
  for tmp in files:
    if (re.match("output_\d+.xlsx",tmp)):
    #if (re.match("output_\d+.csv",tmp)):
      m = re.search(r'_(\w+).', tmp)
      current_num = int(m.group(1))
      if (max_num_file < current_num):
        max_num_file = current_num
  print(files)
  #print(f"\nYour max number = {max_num_file}")
  result_succ = result_df.to_excel(path + "/"+f"output_{max_num_file+1}.xlsx")
  #result_succ = result_df.to_csv(path + "/"+f"output_{max_num_file+1}.csv")
  if(result_succ):
    return True
  else:
    return False
...
'''
##Параметры симмуляции ВС
starting_val = 5
T = 14400              #Время моделирования в секундах
delta_t = 10            #Период отметки
...
Y_start = 0 #начальная координата Y
max_pos_boost = 6.5 #м/с^2 максимальное ускорение самолёта
max_neg_boost = 3.5 #м/с^2 максималное замедление самолёта
initial_boost = 0
min_speed = 140 #500 км/ч минималная скорость самолёта, чтобы он оторвался от земли
max_speed = 250 #900 км/чмаксимальная скорость самолёта, которую он может достичб
initial_speed = 125 # 450 Км/ч начальная скорость авиалайнера
initial_angle = 1 #Начальный угол к оси Ох
max_angle_change = 20 #Максимальный угол поворота за период
max_angle = 20 # Максимульный угол отклонения от курса
prob_boost_change = 0.1 #Вероятность изменения ускорения ВС
prob_angle_change = 0.05 #Вероятность смены курса

##Параметры погрешностей моделируемой РЛС
#prob_timeshift = 0.05
#prob_missing = 0.025
#prob_dubbling = 0.001
prob_timeshift = 0.5
...
prob_dubbling = 0.05
prob_inaccuracy = 1
inaccuracy_wide_x = 10
inaccuracy_wide_y = 10

##Количество генерируемых троекторий для обучающей выборки
count_of_traces = 1
'''
...
def main():
    ...
    #config.sections()
    NNmodels = {}
    #config["MODELS"]["starting_val"]
    for key, value in config['MODELS']:  
        NNmodels[key] = value
    ...
    #result_df = pd.DataFrame(columns=['X', 'Y', 'time'])
    ...
    #result_df.concat(pd.Series(copy.deepcopy(new_np_trajectory[1])), ignore_index = True)
    ...
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.xlim(0, 4500000)  # Масштаб оси X от 0 до 6
    plt.ylim(-4500000/2, 4500000/2) # Масштаб оси Y от 0 до 30
    plt.show()
